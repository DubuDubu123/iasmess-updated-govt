               ...
               <div class="col-12">
                  ...
                  <div class="box p-5" style="text-align: left;border-radius:10px;padding-left: 25px !important;padding-bottom: 25px !important;background: #fff !important;">
                     <div style="color:black;font-size:18px;margin-top: 20px;">
                                ...
                                <div class="font-weight-600" style="color:black;font-size:17px; display: flex; align-items: center;">
            Book your Room |   <a href="#" onclick="showInfo()">
            <img src="assets/img/Info.png" alt="Information" class="info-icon">Info</a>   <span class="danger-book"></span> 
        </div>
        <div class="info-box" id="infoBox">
            <button class="close-btn" onclick="hideInfo()">X</button>
            <strong>Note:</strong><br>
            1. Members can book for a maximum of 5 days per month, with only 2 rooms allowed per booking (24-hour check-in/check-out policy). Booking for more than 5 days in a month will be restricted.<br><br>
            2. Bookings are valid for 24 hours with a 1-hour grace period. Checkouts after the grace period will incur an additional day's charge based on the guest type selected.<br><br>
            3. Please check the 'Rooms Availability' tab for available dates and book accordingly, or alternatively, make booking through the State Guest House website at <a href="http://sgh.tn.gov.in" target="_blank">sgh.tn.gov.in</a>.
        </div>
                           ...
                           <div class="row" style="
                              ...
                              <div class="col-md-12" style="
                                 ...
                                 <div class="form-group">
                                  <div class="col-12">
                @if($isDeleted == 0)
                    <button class="btn btn-primary btn-sm pull-right m-5 check_availabiity" type="button" style="padding: 10px; cursor: pointer;">Check Availability</button>
                    <button class="btn btn-primary pull-right btn-sm m-5 book-now" type="button" disabled style="color:white;">Book Now</button>
                @else
                    <p class="text-danger" style="font-size: 16px;"><span class="danger-book bg-warning">You are not allowed to book.</span></p>
                @endif
            </div>
                                 </div>
                              </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#from_date').on('change', function() {
            ...
            $(".book-now").hide();  // Hide "Book Now" button initially
        ...
        $('#room').on('keyup', function(e) {
            ...
            @if(auth()->user()->hasRole('super-user'))
            var total_roomss = '{{$room_count}}';
            if (numRooms > total_roomss) { 
                e.target.value = total_roomss;
            }
            @else
            if (numRooms > 2) { 
                e.target.value = 2;
        ...
        $.ajax({
            ...
            success: function(res) { 
                if(res.status) {
                    ...
                    popup_data(` 
                        <div class="popup-card"> 
                            <div class="popup-card-content" style="text-align: center;"> 
                                ...
                                <h4 style="font-weight: 600;">${res.message}</h4>
                    ...
                    $(".book-now").hide();  // Hide "Book Now" button if booking fails
                    ...
                    success: function(res) { 
                        ...
                        if(res.status) {
                            ...
                            for(var i=1;i<=room;i++) {
                                var selectedGuestType = $(`select[name="guest_type[${i}]"]`).val() || 'self'; // Maintain selected guest type
                                html_data += `<tr>
                                    <td> ${i}</td>
                                    <td>
                                        <select name="guest_type[${i}]" class="form-control" required="" style="padding:10px;height: 40px;"> 
                                            <option value="self" ${selectedGuestType === 'self' ? 'selected' : ''}>Self</option>
                                            <option value="family" ${selectedGuestType === 'family' ? 'selected' : ''}>Family</option>
                                            <option value="guest" ${selectedGuestType === 'guest' ? 'selected' : ''}>Guest</option>
                                        </select> 
                                    </td> 
                                </tr>`;
                            }
                            ...
                            $(".book-now").show();  // Show "Book Now" button if rooms are available
                            ...
                            $(".book-now").hide();  // Hide "Book Now" button if rooms are not available
                            $(".danger-book").addClass("error");
                            $(".danger-book").removeClass("active");
                            $(".danger-book").html(res.message);
                        } 
...
$(function() {
    ...
    $('#search').on('click', function(e) {
        ...
        $.get('types/fetch', {search: search_keyword}, function(html) {
            $('#js-types-partial-target').html(html);
        });
    });
});
