...
">{{$booking->user->address}} </span></p>
                                ...
                                <div class="bio-row">
                                    <p><span>Booked By </span>: <span>{{$booking->booked_user->name}}</span> </p>
                                </div>
                                <div class="bio-row">
...
<br> 
    ...
    <label for="image" style=" font-size: 16px;
                                ...
                                <button class="confimr btn btn-custom waves-effect waves-light make-confirm" data-url="{{url('/')}}/party/confirm-checkin/{{$booking->id}}" type="button" style="background:#0A7E8C; 
                                        padding-left: 30px;padding-right: 30px;color:white">Confirm</button>
                                         <button class="confimr1 btn btn-custom waves-effect waves-light  common-confirm send-payment-link" data-url="{{url('/')}}/party-booking/send-payment-link/{{$booking->id}}" type="button" style="padding-left: 30px; padding-right: 30px; color:white;background-color:#0A7E8C;display:none">Send Payment Link</button>
                                </div>
                            <br> 
                                            </div>
                                            </form>
        ...
        //                     } else {
        //                         $(".amount-data").hide();
        // $("#number").val('');  // Clear the input box
        // $(".response-error").hide();  // Hide the error message
        ...
        $("#amount").change(function() {
                // alert("Edcedcedc");
                var currentTotalPrice = '{{$booking->tariff}}';
                if ($(this).is(':checked')) {
...
$(document).on('click', '.confimr1', function(e) {
    ...
    if ($("#amount").is(':checked') === true) {
        let totalPrice = parseFloat("{{$booking->tariff}}");
        let advanceAmount = parseFloat($("#number").val());
