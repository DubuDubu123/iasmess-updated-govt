    ...
    <div class="container" style=" width: 450px;">
        ...
        ">
            ...
            <tr>
                ...
                <td style="padding: 10px;border: 1px solid #ddd;text-align: right;">Room</td>
            </tr>
            <tr>
                @if($booking->status == 3)
                  @if($invoice->additional_charge > 0)
                    <td style="padding: 10px; border: 1px solid #ddd;">Amount To be Paid</td>
                <td style="padding: 10px;border: 1px solid #ddd;text-align: right;font-size: 15px;font-weight: bold;">₹ {{$booking_price->amount_need_to_paid + $invoice->additional_charge}}</td>
                  @else
                    <td style="padding: 10px; border: 1px solid #ddd;">Amount To be Paid</td>
                <td style="padding: 10px;border: 1px solid #ddd;text-align: right;font-size: 15px;font-weight: bold;">₹ {{$booking_price->amount_need_to_paid}}</td>
                  @endif
              
                @else
                <td style="padding: 10px; border: 1px solid #ddd;">Amount To be Paid (Advance Amount)</td>
                <td style="padding: 10px;border: 1px solid #ddd;text-align: right;font-size: 15px;font-weight: bold;">₹ {{$booking_price->initial_price}}</td>
                @endif
