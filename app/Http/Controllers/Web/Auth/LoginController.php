...
class LoginController extends ApiController
{
    ...
    $key = 'adUvN85GyRzlWvBx'; // 16 bytes key
    $iv = '1234567890abcdef'; // 16 bytes IV
    $data = base64_decode($request->password);
    // dd($data);
    // Convert the key to binary (it should already be 16 bytes)
    $key = hex2bin(bin2hex($key));

    // Decrypt using AES-128-CBC
    $password = openssl_decrypt($data, 'aes-128-cbc', $key, OPENSSL_RAW_DATA, $iv); 
        // dd($password);
        if (method_exists($this, $method = 'resolveUserFrom' . Str::studly($identifier))) {
            // dd("DSdsfsdfsf");
            // dd($method);
     ...
     public function decryptData($encryptedData, $key) {
        $iv = '1234567890abcdef'; // IV must be 16 bytes long
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function loginUserWithSocialUniqueId($request, $role, $needsToken = true, array $conditions = [])
    {
        $user = null;
        $identifier = $this->getLoginIdentifier();
