...
{ 
    ...
    $query = RoomBooking::query()
                        ->join('users as user_creator', 'room_booking.user_id', '=', 'user_creator.id')
                        ->select('room_booking.*', 'user_creator.name as user_name');

    if (!auth()->user()->hasRole('super-user') && !auth()->user()->hasRole('mess-manager')) {
        $query->where('room_booking.user_id', auth()->user()->id);
    } 
    ...
    if ($request->date_value) {
        switch ($request->date_value) {
            case 1:
                ...
                break;
            case 2:
                $query->whereDate('checkin_date', Carbon::today('Asia/Kolkata'));
                break;
            case 3:
                $query->whereBetween('checkin_date', [Carbon::now('Asia/Kolkata')->startOfWeek(), Carbon::now('Asia/Kolkata')->endOfWeek()]);
                break;
            case 4:
                $query->whereBetween('checkin_date', [Carbon::now('Asia/Kolkata')->startOfMonth(), Carbon::now('Asia/Kolkata')->endOfMonth()]);
                break;
            case 5:
                $query->whereBetween('checkin_date', [Carbon::now('Asia/Kolkata')->startOfYear(), Carbon::now('Asia/Kolkata')->endOfYear()]);
                break;
        }
    }
    ...
    // Apply user filter
    if ($request->user) {
        $query->where('room_booking.user_id', $request->user);
    }

    // Order the results
    $query->orderBy('room_booking.created_at', 'DESC');
...
{
    // Initialize the query and view name based on the type
    $query = null;
    $view_name = null;

    switch ($request->type) {
        case 1:
            $query = RoomBooking::query()->join('users as user_creator', 'room_booking.user_id', '=', 'user_creator.id')
                                         ->select('room_booking.*', 'user_creator.name as user_name');
            $view_name = '_types1';
            break;
        case 2:
            $query = PartyBooking::query()->join('users as user_creator', 'party_booking.user_id', '=', 'user_creator.id')
                                          ->select('party_booking.*', 'user_creator.name as user_name');
            $view_name = '_types2';
            break;
        case 3:
            $query = SportsBooking::query()->join('users as user_creator', 'sports_booking.user_id', '=', 'user_creator.id')
                                           ->select('sports_booking.*', 'user_creator.name as user_name');
            $view_name = '_types3';
            break;
        default:
    ...
    // Filter by user role if the user has the 'user' role
    ...
    if ($request->has('data')) {
        ...
        if (auth()->user()->hasRole('user')) {
            $query->whereBetween('checkin_date', [Carbon::now('Asia/Kolkata')->subMonths(6)->startOfMonth(), Carbon::now('Asia/Kolkata')->endOfDay()]);
        } else {
            $query->whereDate('checkin_date', Carbon::today('Asia/Kolkata'));
        }
    }

    // Order the results and apply limit
