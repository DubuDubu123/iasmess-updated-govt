...
class NewDashboardController extends BaseController
{
    ...
    { 
        ...
        if(auth()->user()->hasRole("user"))
        {
            ...
            //                     ->count();
            $month_earnings[] = $query->where('user_id',auth()->user()->id)->whereDate('checkin_date', '>=', $sixMonthsAgo)->count();
            $month_earnings[] = $party_query->where('user_id',auth()->user()->id)->whereDate('checkin_date', '>=', $sixMonthsAgo)->count();
            $month_earnings[] = $sports_query->where('user_id',auth()->user()->id)->whereDate('checkin_date', '>=', $sixMonthsAgo)->count();
        }
        else{ 
            // dd($fromDate);
            // dd($toDate); 
            ...
            ->where('rb.actual_checkin_date', '>=', $fromDate)
            ->where('rb.actual_checkout_date', '<=', $toDate)
            ->where('rb.is_paid',1)
            // ->get();
            ->sum('rbp.total_price');
            $month_earnings[] = DB::table('party_booking') 
    ...
    public function export(httprequest $request)
    {
        ...
        switch($type)
        {
            case 1:  
                if(auth()->user()->hasRole("user"))
                {
                    $booking = RoomBooking::where('user_id',auth()->user()->id)->where('is_paid',1);
                    $party = PartyBooking::where('user_id',auth()->user()->id)->where('is_paid',1);
                    $sports = SportsBooking::where('user_id',auth()->user()->id)->where('is_paid',1);
                }
                else{
            ...
            case 2:
                if(auth()->user()->hasRole("user"))
                {
                    $query = RoomBooking::where('user_id',auth()->user()->id)->where('is_paid',1);
                }
                else{
