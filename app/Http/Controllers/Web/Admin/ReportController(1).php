...
// dd($statuses);
    ...
    // Debugging SQL query and bindings
...
public function downloadUserReport(Request $request)
{
    ...
    // Get filtered data using direct table queries
    $data = $this->getFilteredData($request);
    
    // Generate a file name
    $filename = "Officer_Report_" . date('ymdis') . '.' . $format;

    // Store the Excel file using the filtered data
...
public function export_pdf(Request $request)
{
    ...
    // Log the data count to verify it's filtering correctly
    \Log::info('Data count for PDF export: ' . $data->count());
