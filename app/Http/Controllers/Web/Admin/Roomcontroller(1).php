...
use App\Jobs\Notifications\Auth\Registration\BookingNotification;
...
use Mail;
    ...
    public function __construct(User $user, OTPHandlerContract $otpHandler, SMSContract $smsContract,ImageUploaderContract $imageUploader)
    {
     ...
     public function send_payment_link($booking) 
    {
        ...
        // Generate a unique token for the payment link
            $token = Str::random(32);
            $booking_data->update(['payment_link' => $token]);
            ...
            Mail::to($user->email)->send(new BookingNotification($details));
...
public function index()
{
    ...
    return view('admin.types.index', compact('page', 'main_menu', 'sub_menu', 'sub_menu_1', 'user', 'isDeleted'))->render();
}
      ...
      public function cancel_booking(RoomBooking $booking)
        {
            ...
            $message1="IAS+Officers%27+Mess%3A+Dear+".urlencode($user->name)."%7D%2C+With+reference+to+your+Booking+ID+".$booking->booking_id."%2C+we+regret+to+inform+you+that+your+booking+has+been+cancelled+as+per+your+request+-+TNPROT";
            $response = $this->smsContract->whatsappsend($phoneNumber, $message1); // Changed from $input to $mobile
